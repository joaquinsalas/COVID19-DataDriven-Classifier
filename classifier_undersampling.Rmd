...
for (num.classifier in c(1:unique.rows)) {
  ...
  for (trial in seq(1,cv.trial)) {
    ...
    negative.balanced.ind <- sample(negative, size = length(positive))
    
    train.balanced = rbind(train[positive,], train[negative.balanced.ind,])
  ...
  filename = sprintf("roc_SS_undersampling_%03d_%03d.csv", trial, num.classifier)
  write.csv(roc$curve,filename)
  
  filename = sprintf("pr_SS_undersampling_%03d_%03d.csv", trial, num.classifier)
